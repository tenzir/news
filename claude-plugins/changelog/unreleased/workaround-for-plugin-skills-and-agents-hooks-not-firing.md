...
The `link-plugin-components.sh` script copies plugin components that have hooks defined in their frontmatter to the project's `.claude/` directory on startup. It reads the `settings.json` to discover all enabled plugins and processes each one, updating the `name:` field to include the plugin prefix and resolving `$CLAUDE_PLUGIN_ROOT` paths to absolute paths.
